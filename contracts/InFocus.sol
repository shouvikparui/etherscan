...
contract InFocus is ERC721URIStorage{
    ...
    function createCampaign(string memory _url,string memory _name,uint _target,uint _amtNFT,uint _deadline)public returns(uint256){
       noCampaigns++;
       Campaign storage newCampaign=campaigns[noCampaigns];
       newCampaign.id=noCampaigns;
    ...
    function contribute(uint _id)public payable{
        ...
        newCampaign.noContribution+=1;
         if(msg.value >= newCampaign.amtNFT){
               ...
               noNFT++;
               _mint(msg.sender,noNFT);
               string memory svg4=string(abi.encodePacked(svg1,Strings.toString(noNFT),svg2));
    ...
    function withdraw(uint _id)public{
          ...
          require(campaigns[_id].isAchieved==false,"Already Withdrawn");
    ...
    function postInnovation(string memory _url)public{
        noInnovation++;
        Innovation storage newInnovation=Innovations[noInnovation];
        newInnovation.url=_url;
    
    }
    function postComment(uint _id,string memory _comment)public{
       Innovation storage newInnovation=Innovations[_id];
       newInnovation.comments.push(Comment(msg.sender,_comment));
    }
    function showPost(uint _id)public view returns(Innovation memory) {
        return(Innovations[_id]);
    }
    function showComments(uint _id)public view returns(Comment[] memory){
        return(Innovations[_id].comments);
    }
